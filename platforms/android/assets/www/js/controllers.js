...
.controller('MainController', function($scope, $rootScope, $stateParams, $interval, $ionicGesture, $ionicBackdrop, $timeout, $http, $ionicLoading) {
    ...
    function getDeltaAngleFromDeltaX(deltaX) {
        ...
        if ( deltaX < RADIUS_DIAL )   // 0 to 90 degrees
            return Math.asin( deltaX / RADIUS_DIAL ) * 360 / (2*Math.PI);   // In degrees
        else if ( deltaX == RADIUS_DIAL )   // 90 degrees
            return 90;
        else // 90 to 180 degrees
            return 90 + Math.asin( (deltaX-RADIUS_DIAL) / RADIUS_DIAL ) * 360 / (2*Math.PI);
    }
...
//                dialElem.style['-webkit-transition-property'] = 'all';
            ...
            */
            ...
            dialElem.style.webkitTransform = "translate3d(0px, 0px, 0px) rotate(" + $scope.thisRotation + "deg)";
            //dialElem.style['-webkit-transition-duration'] = transitionTime + 's';
            //dialElem.style['-webkit-transition-property'] = 'all';
